...
def check_calendar_availability(start: str, end: str, tool_context: ToolContext) -> dict:
    """指定した時間帯に重なる予定を Google Calendar API から取得し、空き状況を返す。

    重なる予定がなければ空きあり、あればその予定を返す。

    Args:
        start: 候補時間帯の開始日時（ISO 8601 形式。例 "2026-09-20T10:00:00+09:00"）
        end: 候補時間帯の終了日時（ISO 8601 形式。例 "2026-09-20T11:00:00+09:00"）

    Returns:
        dict: 以下のいずれかを返す。
            - {"status": "ok", "available": True, "events": []} : 空きあり
            - {"status": "ok", "available": False, "events": [{"summary": "...", "start": "...", "end": "..."}]} : 予定あり
            - {"status": "error", "message": "..."} : トークン未取得または API エラー
    """
    # 関数「authenticate」で state にセットしたアクセストークンを取得
    # アクセストークンを使って Calendar API を実行する
    token = tool_context.state.get("access_token")
    if not token:
        return {"status": "error", "message": "アクセストークンが state に見つかりません。"}

    # 候補時間帯に重なる予定を取得
    # 利用API：Method: events.list
    # https://developers.google.com/workspace/calendar/api/v3/reference/events/list?hl=ja
    # timeMin / timeMax で期間を指定すると、その期間に重なる予定だけが返る
    # singleEvents=true: 繰り返し予定を1件ずつの予定に展開して返す
    resp = requests.get(
    ...
    if resp.status_code != 200:
        return {"status": "error", "message": f"Calendar API エラー ({resp.status_code}): {resp.text[:300]}"}

    # 重なっている予定を件名・開始・終了だけの形に整える
    # ※ 時間指定の予定は dateTime、終日予定は date に値が入る
    events = [
        {
            "summary": e.get("summary", "（件名なし）"),
            "start": e.get("start", {}).get("dateTime") or e.get("start", {}).get("date", ""),
            "end": e.get("end", {}).get("dateTime") or e.get("end", {}).get("date", ""),
        }
        for e in resp.json().get("items", [])
    ]
    logging.info("check_calendar_availability: %s〜%s に %d 件の予定", start, end, len(events))

    # 重なる予定が1件もなければ空きあり
    return {"status": "ok", "available": not events, "events": events}
...
# 第3エージェント: カレンダーを検索して予定の空き状況を確認する
calendar_agent = Agent(
    name="calendar_agent",
    model=MODEL,
    description="Google カレンダーを確認し、候補日時の空き状況を返す。",
    instruction=(
        "summarizer_agent が抽出した候補日時について、カレンダーの予定を確認して空き状況を返してください。\n"
        "\n"
        "【手順】\n"
        "1. 候補日時ごとに check_calendar_availability ツールを呼び出す\n"
        "   - start / end は ISO 8601 形式（例 2026-09-20T10:00:00+09:00）に変換して渡す\n"
        "   - タイムゾーンは日本時間（+09:00）とする\n"
        "2. ツールの結果をもとに、候補日時ごとに以下のフォーマットで出力する\n"
        "   - available が true の場合： - YYYY-MM-DD（曜日） HH:MM〜HH:MM → ⭕ 空きあり\n"
        "   - available が false の場合： - YYYY-MM-DD（曜日） HH:MM〜HH:MM → ❌ 予定あり（（件名） HH:MM〜HH:MM）\n"
        "3. 候補日時が提示されていない場合は「候補日時が提示されていないため空き状況を確認できません。」と伝える"
    ),
    tools=[check_calendar_availability],
)


# 「OAuth認証」 → 「日程調整メール取得・判別」 → 「フォーマット整形・出力」 → 「カレンダー空き状況確認」 の順に処理するオーケストレーター
...
# calendar_agent：summarizer_agent が抽出した候補日時について、カレンダーの空き状況を確認する
root_agent = Agent(
    name="root_agent",
    model=MODEL,
    # エージェントの説明
    ...
    instruction=(
        ...
        "4. summarizer_agent が抽出した候補日時を calendar_agent ツールに渡し、空き状況を確認する\n"
        "   - 確認結果は summarizer_agent の出力に続けて【空き状況】として出力する"
    ),
